...
init_stock <- function(init.pop.size,stock_name){
  age <- rep(4, init.pop.size)
  ...
  data.frame(
    id, number, cohort, age, sex, mother, father,
    stock = stock_name,
    stringsAsFactors = FALSE
  )
}

stocks <- list(GMZ1 = init_stock(1000,"GMZ1"), GMZ2 = init_stock(5000,"GMZ2"))


pop_list <- lapply(stocks, function(x) vector("list", run.years + 1))
caught_list <- lapply(stocks, function(x) vector("list", run.years))


for (yr in 1:run.years) {
  
  for (s in names(stocks)) {
    
    current_pop <- stocks[[s]]
    
    # template for offspring
    pop.year0 <- data.frame(
      id     = character(),
      number = integer(),
      cohort = integer(),
      age    = integer(),
    ...
    offspring_df <- pop.year0[0, ]   # empty by default
    
    ## 1. Age
    current_pop$age <- current_pop$age + 1
    
    ## 2. Reproduction
    mothers <- current_pop %>% filter(sex == "1", age >= rep.age)
    fathers <- current_pop %>% filter(sex == "0", age >= rep.age)
    ...
    if (nrow(mothers) > 0 && nrow(fathers) > 0) {
      ...
      if (!is.null(offspring_df) && nrow(offspring_df) > 0) {
        ...
        offspring_df$id <- paste(offspring_df$cohort,
                                 offspring_df$number,
                                 offspring_df$sex,
                                 sep = "_")
        offspring_df <- offspring_df[, names(pop.year0)]
      } else {
        offspring_df <- pop.year0[0, ]
      }
    }
    ...
    ## --- 5. Save population ---
    pop_list[[s]][[yr + 1]] <- current_pop
    
    ## --- 6. Update stock ---
    stocks[[s]] <- current_pop
  }
  
  ## --- 7. OPTIONAL: mixing between stocks ---
  ## e.g., 5% of GMZ2 moves into GMZ1
  # mix_n <- round(0.05 * nrow(stocks$GMZ2))
  # migrants <- sample_n(stocks$GMZ2, mix_n)
  # migrants$stock <- "GMZ1"
  # stocks$GMZ1 <- bind_rows(stocks$GMZ1, migrants)
  # stocks$GMZ2 <- anti_join(stocks$GMZ2, migrants, by="id")
}
...
stock.sim <- function(init.pop.size.1, init.pop.size.2){
